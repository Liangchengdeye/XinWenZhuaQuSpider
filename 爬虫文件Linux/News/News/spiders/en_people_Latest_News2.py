# -*- coding: utf-8 -*-
# en_people.Latest_News1 首页下部分内容网爬取内容
# create 2017/9/21 W_H_J

import scrapy
from News.items import en_people_latest2
from News.dbhelper import DBHelper
class en_people_Latest_News2(scrapy.Spider):
        name = "en_people_Latest_News2"
        # 爬虫爬取的数据范围
        allowed_domains = ['en.people.cn']
        # 需要拼接的url
        baseURL = "http://en.people.cn/202936/"
        offset = 1
        # 启示地址
        ...
        def parse(self, response):
            ...
            sql_latest2 = "select news_id from news"  # 单独查询编辑
            str_data = data_seltct.select(sql_latest2)  # 执行sql语句
            node_list = response.xpath("//div[@class='d2p3_left fl']")
            # 循环读取到每条新闻Title并获取链接
            for node in node_list:
                item = en_people_latest2()
                for i in range(len(node.xpath("//h3/a/text()").extract())):

                    item['latest2_title'] = node.xpath("//h3/a/text()").extract()[i].encode('utf-8')
                    str_url = "http://en.people.cn"+node.xpath("//h3/a/@href").extract()[i].encode('utf-8')
                    item['latest2_url'] = str_url
                    # 生成新闻ID
                    str_index = str_url.index("n3")
                    str_Id = str_url[str_index + 2:-5].replace('/', '').replace('-', '').replace('c', '')
                    item['latest2_id'] = str_Id
                    # 发布时间
                    if "n3" in str_url:
                        str_index = str_url.index("n3")
                        str_Id = str_url[str_index + 2:-5].replace('/', '').replace('-', '').replace('c', '')
            ...
            if response.xpath("//div[@class='d2_18 clear']/a/text()").extract()[-1] == "Next":
                if response.xpath("//a[@class='common_current_page']/text()").extract()[0] == "1":
                    ...
                    yield scrapy.Request("http://en.people.cn/202936/" + url, callback=self.parse)
                else:
