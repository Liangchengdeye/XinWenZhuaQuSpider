# -*- coding: utf-8 -*-
# en_people.Latest_News1 首页上部分内容网爬取内容
# create 2017/9/21 W_H_J

import scrapy
from News.items import en_people_latest
from News.dbhelper import DBHelper

class en_people_Latest_News1(scrapy.Spider):
        name = "en_people_Latest_News1"
        # 爬虫爬取的数据范围
        allowed_domains = ['en.people.cn']
        # 需要拼接的url
        baseURL = "http://en.people.cn/102842/"
        offset = 1
        # 启示地址
        ...
        def parse(self, response):
            ...
            sql_latest1 = "select news_id from news"  # 单独查询编辑
            str_data = data_seltct.select(sql_latest1)  # 执行sql语句
            node_list=response.xpath("//div[@class='d2p3_left fl']")
            # 循环读取到每条新闻Title并获取链接
            for node in node_list:
                item = en_people_latest()
                for i in range(len(node.xpath("//h3/a/text()").extract())):

                    item['latest_title'] = node.xpath("//h3/a/text()").extract()[i].encode('utf-8')
                    ...
                    if item["latest_id"] in str_data:
                        ...
                        params = (item["latest_id"], item["latest_title"], item["latest_url"], item["latest_time"], '1')
                        data_seltct.insert(sql, *params)  # *表示拆分元组，调用insert（*params）会重组成元组
            # 产生回溯模拟用户Next一直到结束
            if response.xpath("//div[@class='d2_18 clear']/a/text()").extract()[-1] == "Next":
                if response.xpath("//a[@class='common_current_page']/text()").extract()[0] == "1":
                    ...
                    yield scrapy.Request("http://en.people.cn/102842/" + url, callback=self.parse)
                else:
