...
import scrapy
from News.items import en_people_militart
from News.dbhelper import DBHelper
class en_people_Military(scrapy.Spider):
    name = "en_people_Military"
    # 爬虫爬取的数据范围
    allowed_domains = ['en.people.cn']
    # 需要拼接的url
    baseURL = "http://en.people.cn/90786/"
    offset = 1
    # 启示地址
    ...
    def parse(self, response):
        ...
        if response.xpath("//div[@class='d2_18 clear']/a/text()").extract()[-1] == "Next":
            if response.xpath("//a[@class='common_current_page']/text()").extract()[0] == "1":
                ...
                yield scrapy.Request("http://en.people.cn/90786/" + url, callback=self.parse)
            else:
