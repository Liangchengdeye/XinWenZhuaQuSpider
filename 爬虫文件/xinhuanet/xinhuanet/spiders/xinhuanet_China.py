# -*- coding: utf-8 -*-
# xinhuanet_china网爬取内容
# create 2017/9/27 W_H_J

import scrapy
from xinhuanet.items import xinhuanet_china
import json
from xinhuanet.dbhelper import DBHelper

class xinhuanet_China(scrapy.Spider):
    name = "xinhuanet_China"
    pgnum = 1
    start_urls = ["http://www.xinhuanet.com/english/china/index.htm"]
    ...
    def parse_ask(self, response):
        ...
        if data_bool == 0:
            ...
            for data in data_list:
                ...
                item["china_url"] = str_url
                item["china_author"] = str_author
                item["china_pubtime"] = str_pubtime
                item["china_img_arr"] = str_img_arr
                item["china_keyword"] = str_keyword
                yield item
                # 插入数据
